...
if(isset($_POST['form4'])) {
    // updating the database
    $statement = $pdo->prepare("UPDATE tbl_settings SET receive_email=?, receive_email_subject=?,receive_email_thank_you_message=?, forgot_password_message=? WHERE id=1");
    $statement->execute(array($_POST['receive_email'],$_POST['receive_email_subject'],$_POST['receive_email_thank_you_message'],$_POST['forgot_password_message']));
...
if(isset($_POST['form7_3'])) {
    ...
    if($valid == 1) {
        ...
        $result = $statement->fetchAll(PDO::FETCH_ASSOC);                           
        foreach ($result as $row) {
            $banner_forgot_password = $row['banner_forgot_password'];
            unlink('../assets/uploads/'.$banner_forgot_password);
        }

        // updating the data
        $final_name = 'banner_forgot_password'.'.'.$ext;
        move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );

        // updating the database
        $statement = $pdo->prepare("UPDATE tbl_settings SET banner_forgot_password=? WHERE id=1");
        $statement->execute(array($final_name));

        $success_message = 'Forgot Password Page Banner is updated successfully.';
...
foreach ($result as $row) {
   ...
   // $contact_fax                     = $row['contact_fax'];
    ...
    $forgot_password_message         = $row['forgot_password_message'];
                        ...
                        <div class="tab-pane" id="tab_4">
                            ...
                            <div class="box box-info">
                                <div class="box-body">
                                    ...
                                    <div class="form-group">
                                        <label for="" class="col-sm-3 control-label">Forgot password Message</label>
                                        <div class="col-sm-8">
                                            <textarea class="form-control" name="forgot_password_message"><?php echo $forgot_password_message; ?></textarea>
                                        </div>
                                    </div>
                                    <div class="form-group">
                        ...
                        <div class="tab-pane" id="tab_7">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tr>
                                    <form action="" method="post" enctype="multipart/form-data">
                                    <td style="width:50%">
                                        <h4>Existing Forgot Password Page Banner</h4>
                                        <p>
                                            <img src="<?php echo '../assets/uploads/'.$banner_forgot_password; ?>" alt="" style="width: 100%;height:auto;">   
                                        </p>                                        
                                    </td>
                                    <td style="width:50%">
                                        <h4>Change Forgot Password Page Banner</h4>
                                        Select Photo<input type="file" name="photo">
